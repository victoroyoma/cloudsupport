...
export const Documentation = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Searching docs for:', searchTerm);
  };

  const handleFeedback = (isHelpful: boolean) => {
    setShowFeedbackForm(true);
    console.log('Initial feedback:', isHelpful ? 'helpful' : 'not helpful');
  };

  const submitFeedback = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Submitting feedback:', feedbackComment);
  ...
  return <div className="space-y-6">
    <div className="flex items-center justify-between">
      ...
      <form onSubmit={handleSearch} className="relative">
        ...
        <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Search documentation..." />
      </form>
    </div>
          ...
          <div className="px-6 py-5">
            ...
            <div className="flex items-center justify-between border-t pt-4">
              ...
              <div className="flex flex-col items-end space-y-2">
                {!showFeedbackForm ? <div className="flex items-center space-x-2">
                  <span className="text-sm text-gray-500">Was this helpful?</span>
                  <button onClick={() => handleFeedback(true)} className="p-1 rounded-full hover:bg-gray-100">
                    <ThumbsUpIcon className="h-5 w-5 text-gray-400 hover:text-green-500" />
                  </button>
                  <button onClick={() => handleFeedback(false)} className="p-1 rounded-full hover:bg-gray-100">
                    <ThumbsDownIcon className="h-5 w-5 text-gray-400 hover:text-red-500" />
                  </button>
                </div> : <form onSubmit={submitFeedback} className="w-full max-w-lg">
                  {showFeedbackSuccess ? <div className="bg-green-50 border border-green-200 text-green-600 rounded-md p-3 text-sm">
                    Thank you for your feedback!
                  </div> : <>
                    <textarea value={feedbackComment} onChange={e => setFeedbackComment(e.target.value)} placeholder="Tell us more about your experience..." className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" rows={3} />
                    <div className="mt-2 flex justify-end space-x-2">
                      <button type="button" onClick={() => setShowFeedbackForm(false)} className="px-3 py-1 border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-50">
                        Cancel
                      </button>
